//
//  assignment8.cpp
//  This is Pong... with a little bit of juice
//
...
float screenShakeIntensity = 0.05;
float perlinValue = 0;
ParticleEmitter ballEm(200);
...
void Setup() {
	...
	displayWindow = SDL_CreateWindow("This is Pong... with a little bit of juice", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 600, SDL_WINDOW_OPENGL);
...
void SetValues(bool right_won = true) {
    ...
    ballEm.position.x = ball.x;
    ballEm.position.y = ball.y;
}
...
void Update(float elapsed) {
    ...
    if (screenShakeTimer > 0) {
        screenShakeValue += elapsed / 2;
        screenShakeTimer -= SCREEN_TIMER_DEC;
    }
    else {
        screenShakeTimer = 0;
        ballEm.velocityDeviation = 1;
    }
}
